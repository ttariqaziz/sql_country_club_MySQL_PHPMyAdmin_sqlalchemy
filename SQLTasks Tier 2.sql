...
SELECT COUNT(f.name) AS number_of_facilities, Membercost
FROM Members as m
INNER JOIN Bookings as b
ON m.memid = b.memid
INNER JOIN Facilities as f
ON b.facid = f.facid
WHERE Membercost = 0 AND m.memid != 0;
...
SELECT f.name, membercost, monthlymaintenance 
FROM Members as m
INNER JOIN Bookings as b
ON m.memid = b.memid
INNER JOIN Facilities as f
...
SELECT name, monthlymaintenance, 
	CASE WHEN monthlymaintenance > 100 THEN 'Expensive' ELSE 'Cheap' END AS maintenance_type
FROM Facilities;
...
ON m.memid = b.memid
    INNER JOIN Facilities as f
ON b.facid = f.facid
WHERE m.memid != 0 AND f.name LIKE 'Tennis%'
ORDER BY member;
